...
export const colorStore = create<ColorState>()(
  persist(() => initialState, {
    name: 'colorStore',
    // Force rehydration to happen immediately
    skipHydration: false,
  })
)
...
export default function useColorStore(theme: string = 'light') {
  ...
  // Ensure colorState is defined before proceeding
  if (!colorState) {
    return {
      availableColors,
      cssColors: availableColors[0].root,
      color: availableColors[0],
      getColor: () => availableColors[0],
      setColor: () => {},
      updateCssVariables: () => {},
      colorState: initialState,
    }
  }
  
  const getColor = () => {
    // First try to get user's selected color
    if (colorState.userColor) {
  ...
  }, [theme, colorState.userColor, colorState.defaultColor])
  ...
  return {
    ...
    setColor: (name: string, isUserColor?: boolean) => {
      if (isUserColor) {
        colorStore.setState({ userColor: name })
      } else {
        colorStore.setState({ defaultColor: name })
      }
    },
    updateCssVariables,
    colorState,
