...
export default function useColorStore(theme: string = 'light') {
  ...
  return {
    ...
    updateCssVariables: () => {
      ...
      // Appliquer les couleurs au document root
      for (const [key, value] of Object.entries(colors)) {
        // Convertir hsl en oklch pour correspondre à votre CSS
        if (value.includes('hsl(')) {
          const hslValue = value.replace('hsl(', '').replace(')', '')
          const [h, s, l] = hslValue.split(' ').map(Number)
          // Conversion approximative HSL vers OKLCH
          const oklchValue = `oklch(${l/100} ${s/100} ${h})`
      ...
      // Appliquer aussi la classe de thème au body
      document.documentElement.classList.remove('light', 'dark')
      document.documentElement.classList.add(theme)

      // Forcer un re-render en déclenchant un événement personnalisé
      window.dispatchEvent(new CustomEvent('themeChange', { 
        detail: { theme, colorScheme: color.name } 
      }))

      console.log(
        'Applied colors for theme:',
        theme,
        'color scheme:',
        color.name
      )
...
// Hook pour écouter les changements de thème et forcer les re-renders
