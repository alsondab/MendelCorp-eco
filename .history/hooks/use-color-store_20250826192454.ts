...
export default function useColorStore(theme: string = 'light') {
  ...
  const getColor = () => {
    const userColor = colorState.availableColors.find(
      (c) => c.name === colorState.userColor
    )
    if (userColor) return userColor
    
    const defaultColor = colorState.availableColors.find(
      (c) => c.name === colorState.defaultColor
    )
  ...
  return {
    ...
    color,
    getColor,
    setColor: (name: string, isUserColor: boolean = false) => {
      if (isUserColor) {
    ...
    updateCssVariables: () => {
      ...
      const colors = currentTheme === 'light' ? currentColor.root : currentColor.dark
      ...
      console.log('CSS variables updated:', { theme: currentTheme, color: currentColor.name, colors })
    },
  }
}
