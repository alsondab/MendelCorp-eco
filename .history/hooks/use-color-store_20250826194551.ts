...
// Initialize CSS variables on store creation
if (typeof document !== 'undefined') {
  const store = colorStore.getState()
  const defaultColor = store.availableColors.find(c => c.name === store.defaultColor)
  if (defaultColor) {
    // Apply default light theme colors
    for (const [key, value] of Object.entries(defaultColor.root)) {
  ...
  return {
    ...
    updateCssVariables: () => {
      ...
      const colors: { [key: string]: string } =
        currentTheme === 'light' ? currentColor.root : currentColor.dark
      
      console.log('Applying colors for theme:', currentTheme)
      console.log('Selected color scheme:', currentColor.name)
      ...
      const appliedVars = getComputedStyle(document.documentElement)
      console.log('Applied CSS variables:')
      for (const key of Object.keys(colors)) {
        const value = appliedVars.getPropertyValue(key)
