...
export const formatError = (error: any): string => {
  // Vérifier si error existe et a les propriétés nécessaires
  if (!error) {
  ...
  if (error.name === 'ZodError' && error.errors && Array.isArray(error.errors)) {
    const fieldErrors = Object.keys(error.errors).map((field) => {
      const errorMessage = error.errors[field]?.message || 'Erreur de validation'
    ...
    return fieldErrors.join('. ')
  } else if (error.code === 11000 && error.keyValue && typeof error.keyValue === 'object') {
    const duplicateField = Object.keys(error.keyValue)[0]
    return `${duplicateField} existe déjà`
  } else {
