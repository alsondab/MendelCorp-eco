...
const CheckoutForm = () => {
  ...
  // Article pour "Buy Now" (si présent)
  ...
  // Récupérer l'article "Buy Now" depuis l'URL si présent
  useEffect(() => {
    if (buyNowParam) {
      try {
        const decodedItem = JSON.parse(decodeURIComponent(buyNowParam))
        setBuyNowItem(decodedItem)
      } catch (error) {
        console.error('Erreur lors du décodage de l\'article Buy Now:', error)
        toast.error('Erreur lors du chargement de l\'article')
      }
    }
  }, [buyNowParam])

  // Utiliser les articles du panier ou l'article "Buy Now"
  const items = buyNowItem ? [buyNowItem] : cartItems
  const itemsPrice = buyNowItem ? buyNowItem.price * buyNowItem.quantity : cartItemsPrice
  const shippingPrice = cartShippingPrice || 0
  const taxPrice = cartTaxPrice || 0
  const totalPrice = buyNowItem ? itemsPrice + shippingPrice + taxPrice : cartTotalPrice
