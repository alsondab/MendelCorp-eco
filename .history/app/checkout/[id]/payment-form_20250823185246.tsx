...
export default function OrderPaymentForm({
  ...
  const handleCashOnDelivery = async () => {
    try {
      ...
      if (response.ok) {
        toast.success('Commande confirmée ! Un email de confirmation a été envoyé à votre boîte mail.')
        router.push(`/account/orders/${order._id}`)
      } else {
        const errorData = await response.json()
        console.error('Erreur API:', errorData)
        toast.error(`Erreur: ${errorData.error || 'Erreur lors de la confirmation de la commande'}`)
      }
    } catch (error) {
