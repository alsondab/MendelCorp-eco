    ...
    switch (type) {
      ...
      default:
        return NextResponse.json(
          {
            error:
              "Type d'email non reconnu. Types disponibles: test, welcome, custom",
          },
          { status: 400 }
        )
    }
    ...
    endpoints: {
      POST: {
        ...
        body: {
          ...
          subject:
            "string (optionnel) - Sujet de l'email (pour type: test, custom)",
          message: "string (optionnel) - Message de l'email (pour type: test)",
          userName: "string (requis pour type: welcome) - Nom de l'utilisateur",
          htmlContent:
            "string (requis pour type: custom) - Contenu HTML de l'email",
          textContent:
            "string (optionnel pour type: custom) - Version texte de l'email",
        },
        examples: {
          test: {
            to: 'test@example.com',
